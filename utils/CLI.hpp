...
namespace qz
{
    ...
    void parse_command_line_args(argparse::ArgumentParser &parser, int argc, char *argv[])
    {
        ...
        parser.add_argument("--purchase_period")
            .default_value(7ULL)
            .scan<'u', uint64_t>()
            .help("Purchase period (applies to TPOP policy)");

        parser.add_argument("--ordering_cost")
            .default_value(100.0)
            .scan<'g', double>()
            .help("Cost of placing an order (applies to EOQ policy)");

        parser.add_argument("--holding_cost")
            .default_value(0.2)
            .scan<'g', double>()
            .help("Annual holding cost rate (applies to EOQ policy)");
        ...
        parser.add_argument("--policy")
            ...
            .help("Purchasing policy: (ROP | TPOP | EOQ)")
            .choices("ROP", "TPOP", "EOQ");
